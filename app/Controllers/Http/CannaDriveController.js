...
class CannaDriveController {
  ...
  async driverWeeklyIncome({ params }) {
    ...
    let data = await Order.query().select('driverId', Database.raw(' sum(deliveryFee) AS total')).whereBetween('created_at', [laterweek, today]).where('driverId', params.id).fetch()
